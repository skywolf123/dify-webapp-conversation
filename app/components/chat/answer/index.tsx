    ...
    return (
      <div className={`${s.itemOperation} flex gap-2`}>
        ...
        <Tooltip selector={`copy-button-${randomString(16)}`} content={'复制'}>
          {OperationBtn({ innerContent: <IconWrapper>{isCopied ? <ClipboardDocumentCheckIcon className="w-4 h-4" /> : <ClipboardIcon className="w-4 h-4" />}</IconWrapper>, onClick: handleCopy })}
        </Tooltip>
        {/* {userOperation()} */}
      </div>
    );
